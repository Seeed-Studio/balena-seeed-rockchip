#!/usr/bin/env bash
# Build balenaOS for the Seeed reComputer RK3576 DevKit.
#
...
readonly MACHINE="recomputer-rk3576-devkit"

BUILD_DIR="build-recomputer-rk3576-devkit"
CLEAN_BUILD=0

usage() {
	cat <<'EOF'
Usage: scripts/build-recomputer-rk3576-devkit.sh [--clean] [--continue] [--development-image] [--disable-kernel-headers] [--build-dir DIR]

Builds balena-image-flasher for recomputer-rk3576-devkit.
  ...
  --build-dir DIR          repository-relative build directory name (default:
                           build-recomputer-rk3576-devkit)
...
copy_image balena-image-flasher-recomputer-rk3576-devkit.balenaos-img
copy_image balena-image-recomputer-rk3576-devkit.balenaos-img
# Host OS docker image: the hostapp-update -f input (OTA payload).
copy_image balena-image-recomputer-rk3576-devkit.docker
