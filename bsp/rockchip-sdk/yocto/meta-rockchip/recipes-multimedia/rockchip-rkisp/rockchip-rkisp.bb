...
# The vendor Makefile build embeds absolute build paths in the shared
# library and server binaries; skip the buildpaths QA check for them.
...
CFLAGS:append = " -D_GNU_SOURCE -Wno-error=implicit-function-declaration"
...
do_compile() {
	...
	oe_runmake ARCH="${ARCH}" \
		...
		TARGET_GPP="${CXX} ${CPPFLAGS} ${LDFLAGS} -Wno-error=cpp -Wno-error=template-id-cdtor" \
