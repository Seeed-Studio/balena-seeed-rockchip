...
/* Some downstream configurations disable procfs while leaving the encoder
 * device-operation table enabled. Keep the table buildable in that case. */
#ifndef CONFIG_ROCKCHIP_MPP_PROC_FS
static inline int rkvenc_dump_session(struct mpp_session *session,
					      struct seq_file *seq)
{
	return 0;
}
