...
static int tty_write(struct platform_device *pdev, const char *s, int count)
{
	...
	if (count > 0) {
		 ...
		 * wake_up_console_thread() (an anti-deadlock measure for
		 * the printk path) is not needed here.  Wake the console
		 * thread unconditionally so no user-visible output can
		 * lose its wakeup.
		 */
